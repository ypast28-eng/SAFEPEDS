...
-- ─── Bootstrap completion marker (one-time route lock) ───
create table if not exists public._pedsafe_bootstrap (
  id int primary key default 1 check (id = 1),
  completed_at timestamptz not null default now(),
  schema_version text not null default '20250708100006'
);

insert into public._pedsafe_bootstrap (id, schema_version)
values (1, '20250708100006')
